          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSchedule} className="space-y-4">
                ...
                <div><Label htmlFor="participant">Participant</Label><Input id="participant" name="participant" required placeholder="Student name" /></div>
                ...
                <CardContent className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-4">
                    <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                      {m.type === "video" ? <Video className="h-4 w-4 text-primary" /> : <Phone className="h-4 w-4 text-primary" />}
                    </div>
                    <div>
                      <p className="font-medium text-sm">{m.title}</p>
                      <p className="text-xs text-muted-foreground">with {m.participantName}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <Calendar className="h-3 w-3 text-muted-foreground" />
                        <span className="text-xs text-muted-foreground">{m.date} · {m.time}</span>
                        <Badge variant="default" className="text-[10px] h-5">upcoming</Badge>
                      </div>
                    </div>
                  </div>
